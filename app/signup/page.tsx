        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#050e05] px-4">
            <main className="flex w-full max-w-md flex-col gap-8">
                <header className="space-y-2 text-center">
                    ...
                    <p className="text-sm font-semibold uppercase tracking-[0.18em] text-emerald-400">Sign Up</p>
                    <h1 className="text-2xl font-bold text-white">
                        Create your <span className="text-emerald-400">KhetiWala</span> account
                    </h1>
                </header>

                <form onSubmit={handleSignup} className="space-y-4 rounded-2xl border border-[#1a2d1a] bg-[#0a160a] p-6 text-sm shadow-xl">
                    ...
                    <div className="space-y-1">
                        ...
                        <input id="password" type="password" placeholder="Min 6 characters" required value={password} onChange={e => setPassword(e.target.value)}
                            className="h-10 w-full rounded-xl border border-[#1a2d1a] bg-[#0d1a0d] px-3 text-sm text-zinc-200 placeholder-zinc-600 outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600" />
                    </div>
                    ...
                    {error && <p className="rounded-lg bg-red-900/20 border border-red-900/40 px-3 py-2 text-xs text-red-400">{error}</p>}
                    {success && <p className="rounded-lg bg-emerald-900/20 border border-emerald-900/40 px-3 py-2 text-xs text-emerald-400">{success}</p>}
                </form>
